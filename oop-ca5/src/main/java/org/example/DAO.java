...
public class DAO {
    ...
    // Author: Ben
    // Return the ID that will be generated by the database for new gems.
    public int getAutoGenId() {
        ...
        if (conn != null) {
            ...
            try {
                ...
                String query = "SELECT MAX(ID) AS ID FROM Gemstones";

                PreparedStatement stmt = conn.prepareStatement(query);
                ResultSet results = stmt.executeQuery();
                System.out.println(results);

                if(results.next()) { // The next ID will be one greater than the previous max.
